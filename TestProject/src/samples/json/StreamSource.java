...
public class StreamSource implements Runnable {
    ...
    protected void processInputStream(InputStream inputStream, int iteration) throws IOException {
        try (BufferedReader br = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            int productId = 0;
            int counter = 0;
            int STRINGS_TO_SEND = 10000 ;
            String timestamp;
            Date currentTimestamp = new java.sql.Timestamp(Calendar.getInstance().getTime().getTime()) ;
            while (counter < STRINGS_TO_SEND) {
            	//Random numbers
            	productId = rn.nextInt(50)+1;
            	timestamp = new java.sql.Timestamp(Calendar.getInstance().getTime().getTime()).toString();
            	
            	Map<String,Object> userData = new HashMap<String,Object>();
            	userData.put("ProductName", "product"+productId);
            	userData.put("price", rn.nextInt(100)+1);
            	userData.put("zipcode", rn.nextInt((99100 - 91800) + 1) + 91800);
            	userData.put("timestamp", timestamp.toString());
                ...
                putRecordRequest.setData(ByteBuffer.wrap(objectMapper.writeValueAsString(userData).getBytes()));
                putRecordRequest.setPartitionKey(Integer.toString(productId));
